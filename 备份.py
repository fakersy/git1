...
(ALPHA, BETA, RHO, Q) = (2.0, 5.0, 0.5, 100)
    ...
    def __update_viscous_gragh(self):
        ...
        for pn in range(pso_num):
            ...
            for i in range(1, city_num):
                ...
                temp_viscous[pn][start][end] += 0.3
                temp_viscous[pn][end][start] = temp_viscous[pn][start][end]

                decide[start][end] = 1
                decide[end][start] = 1

            temp_viscous[pn][self.psos[pn].path[city_num - 1]][self.psos[pn].path[0]] += 0.3
            temp_viscous[pn][self.psos[pn].path[0]][self.psos[pn].path[city_num - 1]] = temp_viscous[pn][self.psos[pn].path[city_num - 1]][self.psos[pn].path[0]]

            decide[self.psos[pn].path[city_num - 1]][self.psos[pn].path[0]] = 1
            decide[self.psos[pn].path[0]][self.psos[pn].path[city_num - 1]] = 1
            ...
            if decide[self.psos[pn].pbest[city_num - 1]][self.psos[pn].pbest[0]] == 0:
                temp_viscous[pn][self.psos[pn].pbest[city_num - 1]][self.psos[pn].pbest[0]] += 0.5
                temp_viscous[pn][self.psos[pn].pbest[0]][self.psos[pn].pbest[city_num - 1]] = temp_viscous[pn][self.psos[pn].pbest[city_num - 1]][self.psos[pn].pbest[0]]
            ...
            if decide[self.best_pso.path[city_num - 1]][self.best_pso.path[0]] == 0:
                temp_viscous[pn][self.best_pso.path[city_num - 1]][self.best_pso.path[0]] += 0.7
                temp_viscous[pn][self.best_pso.path[0]][self.best_pso.path[city_num - 1]] = temp_viscous[pn][self.best_pso.path[city_num - 1]][self.best_pso.path[0]]

        for pn in range(pso_num):
            for i in range(city_num):
                for j in range(city_num):
                    viscous_graph[pn][i][j] += 0.06
