...
(ALPHA, BETA, RHO, Q) = (1.0, 5.0, 0.5, 500.0)
(city_num, pso_num) = (52, 25)
...
viscous_graph = [[[1 for col in range(city_num)] for raw in range(city_num)] for pn in range(pso_num)]
class PSO():
...
class TSP(object):
    ...
    def __update_viscous_gragh(self):
        ...
        for pn in range(pso_num):
            ...
            ran_path = random.sample(range(city_num), city_num)
            for i in range(1, city_num):
                start, end = ran_path[i - 1], ran_path[i]
                temp_viscous[pn][start][end] += Q / 30000
                temp_viscous[pn][end][start] = temp_viscous[pn][start][end]
            for i in range(1, city_num):
                start, end = self.psos[pn].path[i - 1], self.psos[pn].path[i]
                temp_viscous[pn][start][end] += Q / self.psos[pn].total_distance
                temp_viscous[pn][end][start] = temp_viscous[pn][start][end]
            for i in range(1, city_num):
                start, end = self.psos[pn].pbest[i - 1], self.psos[pn].pbest[i]
                temp_viscous[pn][start][end] += Q / self.psos[pn].p_fit
                temp_viscous[pn][end][start] = temp_viscous[pn][start][end]
            for i in range(1, city_num):
                start, end = self.best_pso.path[i - 1], self.best_pso.path[i]
                temp_viscous[pn][start][end] += Q / self.best_pso.total_distance
                temp_viscous[pn][end][start] = temp_viscous[pn][start][end]
        for pn in range(pso_num):
            for i in range(city_num):
                for j in range(city_num):
